...
def extract_printrun(rec, verbose=False):
    ...
    N_PAGES = r'(?<pages>[0-9]+)\s+([Сс]тр|л)\.(?<pagecomment>(\s+и.+?(вклейки|иллюстр|чертежей|илл)\.)|[(](\p{Ll}|[0-9])[^)]+[)])?'
    N_PRINTRUN = r'(\s*(?<printrun>[1-9][0-9 Оо]+)(\s*[(]((?<part>[0-9]+[ —-]+[0-9]+)\s+т(ыс)?\.|[1-9]-й\s+завод\s+(?<part>[0-9]+)?(\s+т(ыс)?\.)?)[)]\.?)?\s*экз\.)'
