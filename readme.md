...
библиографические данные. Сведения обо всех включенных в указатель
изданиях находятся в файле `editions.csv`. К каждому авторскому
изданию добавлено проверенное вручную унифицированное указание на
автора (в этих указаниях устранен разнобой в написании имен авторов и
разведены авторы-однофамильцы). Помимо этого данные дополнены, где
возможно, уникальными идентификаторами (UUID) произведений, в том
числе опубликованных в составе сборников. Данные об авторах и
произведениях, дополняющие и уточняющие библиографические сведения в
указателях, помещены в таблицах `authors.csv` и `work-ids.csv`,
соответственно.
...
указателей (сканирование и распознавание), таблицу уникальных
идентификаторов произведений, а также выступала координатором для
студентов-участников проекта. Е. О. Казакова подготовила данные об
иллюстраторах. Большую помощь в проверке и дополнении сведений об
авторах оказали А. А. Димяненко и И. А. Сергиенко. Огромный вклад в
проверку данных и корректировку ошибок сделали студенты
образовательных программ «Социология и социальная информатика» и
«Филология» НИУ Высшая школа экономики — Санкт-Петербург, работавшие
над базой в рамках студенческих проектов и летних практик. Мы очень
благодарны всем участникам: Кристина Астарян, Дари Батожаргалова,
Мария Борисова, Лиана Валиахметова, Ася Веревкина, Анастасия Власова,
Любовь Вуличенко, Екатерина Додонова, Мария Еремута, Юлия Карпикова,
Наталья Карцева, Юлия Кожевникова, Олеся Кузьмич, Александра
Кульбаева, Роман Лисюков, Диана Лунгу, Ирина Медведева, Никита
Никифоров, Надежда Одушко, Дмитрий Помогаев, Анна Поскребышева,
Виктория Рудич, Злата Рыбакова, Мария Стародубцева, Арина Сысоева,
Екатерина Тулубенская, Юлия Утран, Елизавета Щербакова, Полина Эйлон,
Ирина Юшкова. Светлана Рябова, Александра Митюкова, Анна Арбузова и
Валерия Сидненко, студентки магистерской программы «Цифровые методы в
гуманитарных науках» НИУ ВШЭ внесли огромный вклад в подготовку второй
версии датасета.
...
* `work-ids.csv` — таблица уникальных идентификаторов произведений.
* `README.md` — этот файл.
...
Файл содержит библиографические сведения об изданиях указателя,
послуживших основным источником данных. Колонки в таблице данных:
...
Файл содержит сведения об изданиях, учтенных в указателях «Детская литература».
Колонки в таблице данных: 

* `vol` — идентификатор тома (соответствует идентификаторам из таблиц
  `bibliography.csv` и `work-ids.csv`).
...
Файл содержит сведения об авторах и уникальные идентификаторы авторов
в рамках набора данных. Колонки в таблице:
...
* `death_year` — год смерти автора. В случае расхождения датировок
...
* `languages` — язык произведений автора (для иностранных авторов).

* `comments` — комментарии.

* `source` — источник сведений об авторе (при отсутствии записи в Wikidata). 


### work-ids.csv

Файл содержит уникальные идентификаторы произведений в рамках набора
данных.  Колонки:

* `vol` — идентификатор тома указателя (соответствует таблице
  `editions.csv`).

* `num` — номер библиографической записи в рамках тома (соответствует
  таблице `editions.csv`).

* `author` — автор(ы) произведения. Если у произведения несколько
  авторов, перечислены через точку с запятой.

* `author_std` —  уникальный строковый идентификатор автора в формате
  `ФамилияИО`, соответствует аналогичному полю в файле `authors.csv`.
   Если у произведения несколько авторов, перечислены через точку с запятой.
  
* `text_title` — заглавие произведения в той форме, в которой оно
  приведено в таблице `editions.csv`.
  
* `text_title_preprocessed` — служебное поле для идентификации
  произведений. Заглавие произведения, приведенное к нижнему регистру
  с удаленной пунктуацией.
  
* `uuid` — уникальный идентификатор произведения (UUID).

* `notes` — служебное поле. Пометы, отражающие неокончательный статус
  атрибуции произведения. Наиболее распространенные случаи:
  `collection` — сборник или цикл произведений; `split` — издание
  содержит произведения разных авторов.

* `check_title` — служебное поле. Сходное название, которое может
  относиться к тому же произведению (нормализованная форма).
  
* `check_uuid` — служебное поле. Идентификатор (UUID) произведения,
  указанного в поле `check_title`.


### ChangeLog

* **декабрь 2025, V4** — Расширенная и исправленная версия.
  Основные изменения: 
  * Добавлена таблица `work-ids.csv`, содержащая уникальные
    идентификаторы произведений.
  * В таблице `authors.csv` добавлены колонки `languages`, `comments`,
    `source`.
  * Для 683 авторов установлены идентификаторы Wikidata (qid). Кроме этого, для
    115 авторов, о которых не имеется записи в Wikidata, установлены
    даты жизни.
  * Исправлены ошибки в атрибуции изданий авторам и прочие замеченные ошибки.
